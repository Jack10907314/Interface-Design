...
void MainWindow::TimeTick_Pin()
{
    ...
    if(previous_read_player2_up == 1 && digitalRead(Pin_player2_up) == 0)
    {
        key_player2_up=false;
        player_2.direct_count--;
    }
    if(previous_read_player2_down == 1 && digitalRead(Pin_player2_down) == 0)
    {
        key_player2_down=false;
        player_2.direct_count--;
    }
    if(previous_read_player2_left == 1 && digitalRead(Pin_player2_left) == 0)
    {
        key_player2_left=false;
        player_2.direct_count--;
    }
    if(previous_read_player2_right == 1 && digitalRead(Pin_player2_right) == 0)
    {
...
void MainWindow::addSpeed(int player_num)
{
    if(player_num == PLAYER1)
    {
        player_1.Speed = object_size / 4;
    }

    if(player_num == PLAYER2)
    {
        player_2.Speed = object_size / 4;
    }
}
